...
<br>  
```{r, results = 'asis', fig.align='center', fig.height=5, fig.width=7.5}
ex_p_df2 %>% 
  ggplot(aes(x = value, 
             y = stat,
             group = name)) + 
  geom_point(aes(color = woba_luck_label),#, shape = stat), 
             show.legend = FALSE, 
  ...
  ggrepel::geom_label_repel(data = ex_p_df2 %>% spread(stat, value) %>% mutate(stat = 'Expected - Actual'), 
                            aes(x = xWOBACON, label = woba_luck_label, color = woba_luck_label),
...
walk(teams, function(t){
  ...
  woba_luck_label_ys <- p_df %>% drop_na %>% 
    anti_join(woba_labels) %>% 
    filter(abs(woba_luck) > .1) %>% 
    group_by(woba_luck_label) %>% 
    filter(y_ax_rank == min(y_ax_rank)) %>% 
    ungroup  %>% 
    mutate(mid_woba = (woba_value_mean + estimated_woba_using_speedangle_mean) / 2) 
  ...
  p <- p_df %>% 
    ...
    geom_point(aes(x = estimated_woba_using_speedangle_mean, 
               ...
               show.legend = FALSE, 
               size = 1.25, shape = 21, stroke = rel(1.5), fill = "white") + 
    ggrepel::geom_text_repel(data = woba_labels,
    ...
    ggrepel::geom_label_repel(data = woba_luck_label_ys,
                              aes(x = mid_woba, label = woba_luck_label, color = woba_luck_label),
                              show.legend = FALSE, segment.alpha = .75, nudge_y = -.085) + 
    ggforce::facet_col(~g, space = 'free', scales = 'free_y') + 
